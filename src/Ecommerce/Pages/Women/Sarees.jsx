...
export default function Sarees({ cart, setCart }) {
    const [WomenSarees] = useState(Womenlist.Sarees);
    return(
        <div>
            <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Saree collections for Women</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {WomenSarees.map((product) => (
            <Card
              key={product.id}
