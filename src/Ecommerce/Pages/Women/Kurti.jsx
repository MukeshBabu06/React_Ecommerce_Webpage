...
export default function Kurti({cart, setCart}) {
    const [WomenKurti] = useState(Womenlist.Kurti);
    return(
        <div>
            <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Kurti collection for Women</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {WomenKurti.map((product) => (
            <Card
              key={product.id}
