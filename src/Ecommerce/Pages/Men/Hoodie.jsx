...
export default function Hoodie({ cart, setCart }) {
  const [Menshoodies] = useState(data.hoodies);

  return (
    <div>
      <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Hoodie collections for Men</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {Menshoodies.map((product) => (
            <Card
              key={product.id}
