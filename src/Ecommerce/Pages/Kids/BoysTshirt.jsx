...
export default function BoysTshirt({ cart, setCart }) {
    const [BoyTshirt] = useState(Kidslist.KBoysTshirts);

    return(
        <div>
            <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Boy's T shirts</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {BoyTshirt.map((product) => (
            <Card
              key={product.id}
