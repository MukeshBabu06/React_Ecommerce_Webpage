...
export default function GirlsDress({ cart, setCart }) {
    const [GirlDress] = useState(Kidslist.GirlsDresses);

    return(
        <div>
            <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Girl's Dresses</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {GirlDress.map((product) => (
            <Card
              key={product.id}
