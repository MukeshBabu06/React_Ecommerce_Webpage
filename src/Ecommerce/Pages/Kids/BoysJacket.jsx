...
export default function BoysJacket({ cart, setCart }) {
    const [Boyjacket] = useState(Kidslist.KBoysJacket);

    return(
        <div>
            <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Boy's Jackets</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {Boyjacket.map((product) => (
            <Card
              key={product.id}
