...
function WomenWatch({ cart, setCart}) {
    const [Womanwatch] = useState(watches.WomenWatch);

  return (
    <div>
      <Container fluid className="EPrdlist">
        <h1 className="mt-2 mb-2 text-center">Watches for Women</h1>
        <Row lg={12} className="mt-2 mb-2 justify-content-center align-items-center flex-wrap">
          {Womanwatch.map((product) => (
            <Card
              key={product.id}
